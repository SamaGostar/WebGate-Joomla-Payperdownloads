...
class ModelPayPerDownloadPlusLicenses extends JModel
{ 
	function getUserLicenses($start, $limit)
	{
		...
		if($user && $user->id)
		{
			...
			foreach($licenses as $license)
			{
				...
				$this->getResourcesLinks($license->resources);
	...
	function getResourcesLinks(&$resources)
	{
		...
		{
			$link = "";
			$dispatcher->trigger('onGetLink', array ($resource, &$link));
			if($link)
			{
				$this->addMenuItemParameter($link);
				$resource->link = JRoute::_( $link );
			}
		}
	}
	...
	function getLicenseResources($license)
	{
		if($license)
		{
			...
			$query = "SELECT DISTINCT
				...
				#__payperdownloadplus_resource_licenses.resource_id,
				...
				#__payperdownloadplus_resource_licenses.resource_params,
				#__payperdownloadplus_resource_licenses.resource_option_parameter
