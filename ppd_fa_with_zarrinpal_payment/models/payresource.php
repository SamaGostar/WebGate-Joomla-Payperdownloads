...
class ModelPayPerDownloadPlusPayResource extends JModel
{ 
	var $config = null;
	...
	function handleResponse()
	{
		...
		if(!$dealt && $this->getUsePaypal())
		{
			...
			if($payed)
			{
				...
				$payed = $this->validateResource($resource_id, $payed_price, $currency_code) ? 1 : 0;
				if($payed)
				{
	...
	function setDownloadLinkPayed($download_id, $resource_id, $payer_email, $user_email, $returnUrl)
	{
		...
		$payer_email = $db->getEscaped($payer_email);
		$user_email = $db->getEscaped($user_email);
	...
	function validateResource($resource_id, $payed_price, $currency_code)
	{
