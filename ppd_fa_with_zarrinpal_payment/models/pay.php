...
class ModelPayPerDownloadPlusPay extends JModel
{ 
	var $config = null;
	...
	function getPaymentInfo()
	{
		...
		$paymentInfo->usesimulator = $config->get('usesimulator', 1);
		return $paymentInfo;
	}
	...
	function getLicense($licenseId)
	{
		...
		if(!$result)
		{
			require_once (JPATH_ADMINISTRATOR . DS . "components" . DS . "com_payperdownloadplus" . DS . "classes" . DS . "debug.php");
	...
	function handleResponse()
	{
		...
		if($usePayPlugin)
		{
			...
			if($dealt)
			{
				...
				$txn_id = $db->getEscaped($transactionId);
				$response = $db->getEscaped($response);
				$validate_response = $db->getEscaped($validate_response);
				$status = $db->getEscaped($status);
				$amount = $db->getEscaped((float)$amount);
				$fee = $db->getEscaped((float)$fee);
				$tax = $db->getEscaped((float)$tax);
				$mc_currency = $db->getEscaped($currency);
				$esc_user_email = $db->getEscaped($payer_email);
				$query = "INSERT INTO 
		...
		if(!$dealt && $this->getUsePaypal())
		{
			...
			$status = $db->getEscaped($status);
			$amount = $db->getEscaped(JRequest::getFloat('mc_gross'));
			$fee = $db->getEscaped(JRequest::getFloat('mc_fee', 0));
			$tax = $db->getEscaped(JRequest::getFloat('tax'));
			$mc_currency = $db->getEscaped(JRequest::getVar('mc_currency'));
			$esc_user_email = $db->getEscaped($payer_email);
			$esc_receiver_email = $db->getEscaped($receiver_email);
			$query = "INSERT INTO 
	...
	function validateLicense($license_id, $payed_price, $currency_code, $user_id)
	{
		...
		return ($license && (float)$payed_price - $price >= 0.00 && trim(strtoupper($license->currency_code)) == trim(strtoupper($currency_code)));
	}
	...
	function getPaymentData($payment_id)
	{
		...
		$db->setQuery($query);
		$result = $db->loadObject();
		$this->trace("Get payment data with query : $query");
		if(!$result)
			$this->trace("Failed query: $query");
		return $result;
	}
	...
	function updateDownloadLink($download_id, $subject, $text, $download_link)
	{
		...
		$text = $db->getEscaped($text);
		$download_link = $db->getEscaped($download_link);
	...
	function assignLicense($user_id, $license_id, $credit = 0, $set_credit_days = false)
	{
		...
		if(!$userLicense)
		{
			...
			$this->trace("Create license with query : $query");
			if(!$query_result)
				$this->trace("Failed query : $query");
		}
		else
		{
			...
			$db->setQuery( $query );
			$query_result = $db->query();
			$this->trace("Update license with query : $query");
			if(!$query_result)
				$this->trace("Failed query : $query");
	...
	function getUserLicense($user_id, $license_id)
	{
		...
		$db->setQuery( $query );
		$result = $db->loadObject();
		$this->trace("Get user licenses with query : $query");
		if(!$result)
			$this->trace("Failed query : $query");
		return $result;
	}
	...
	function getDiscountLicense($license, $user_id)
	{
		$level = (int)$license->level;
		$db = JFactory::getDBO();
		$cur = $db->getEscaped($license->currency_code);
		
		$config = $this->getConfig();
		if($config->get('apply_discount', 1) == 0)
		...
		if($license->price > $sum)
			return $license->price - $sum;
		else
			return 0; // free license
	}
	...
	function removeUsedCredit($license, $user_id)
	{
		...
		$query = "SELECT 
			...
			WHERE #__payperdownloadplus_users_licenses.expiration_date > NOW() AND 
				...
				#__payperdownloadplus_users_licenses.user_id = " . (int)$user_id;
		$db->setQuery($query);
		$user_licenses = $db->loadResultArray();
	...
	function createDownloadLink($resource_id, $itemId)
	{
		...
		$e_secret_word = $db->getEscaped($secret_word);
		$e_random_value = $db->getEscaped($random_value);
		if($itemId)
			$dbItemId = "'" . $db->getEscaped($itemId) . "'";
		else
			$dbItemId = "NULL";
