...
require_once(JPATH_COMPONENT.DS.'html'.DS.'vdatabindpanel.html.php');
require_once(JPATH_COMPONENT.DS.'html'.DS.'menuitem.html.php');
require_once(JPATH_COMPONENT.DS.'html'.DS.'vdatabindmodel.html.php');
require_once(JPATH_COMPONENT.DS.'html'.DS.'vdgroupsselect.html.php');
...
class ConfigForm extends PPDForm
{
	...
	*/
	...
	{
		parent::__construct();
		$this->context = 'com_payperdownloadplus.config';
	...
	*/
	function createDataBinds()
	{
		if($this->dataBindModel == null)
		{
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_PAYMENT_PANE"), "Gateway", true, false); 
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_NOTIFICATION_PANE"), "Notification", true, true); 
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_BACKEND_SETTINGS"), "Back-end", true, true); 
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_FRONTEND_PANE"), "Front-end", true, true); 
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new RadioVisualDataBind('redirect_to_login', JText::_('PAYPERDOWNLOADPLUS_REDIRECT_TO_LOGIN'));
			$bind->setEditToolTip(JText::_("PAYPERDOWNLOADPLUS_REDIRECT_TO_LOGIN_DESC"));
			$bind->defaultValue = 0;
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_MENU_ITEMS_PANE"), "Front-end", true, true); 
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_INTEGRATION"), "AUP", true, true); 
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new CPanelSeparatorVisualDataBind(JText::_("PAYPERDOWNLOADPLUS_OTHER_SETTINGS"), "Front-end", true, true); 
			$this->dataBindModel->addDataBind( $bind );
			...
			$bind = new CPanelSeparatorVisualDataBind("", "", false, true); 
			$this->dataBindModel->addDataBind( $bind );
		}
	}
	...
	function edit($task, $option)
	{
		if($this->editItemScripts != null)
		{
			...
			{
				JHTML::script($script, $scriptPath, false);
			}
		}
		$db = JFactory::getDBO();
	...
	function createToolbar($task, $option)
	{
		...
		if($version->RELEASE >= "2.5")
			if (JFactory::getUser()->authorise('core.admin', 'com_payperdownloadplus')) 
			{
				JToolBarHelper::preferences('com_payperdownloadplus');
			}
	}
