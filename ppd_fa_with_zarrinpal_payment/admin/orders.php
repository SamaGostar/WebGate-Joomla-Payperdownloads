...
class OrdersForm extends PPDForm
{
	...
	*/
	...
	{
		parent::__construct();
		$this->context = 'com_payperdownloadplus.payments';
	...
	*/
	function createDataBinds()
	{
		if($this->dataBindModel == null)
		{
			...
			$bind = new CalendarVisualDataBind('payment_date', JText::_('PAYPERDOWNLOADPLUS_PAYMENT_DATE_111'));
			$bind->allowBlank = true;
			$bind->setColumnWidth(10);
			$bind->disabled = true;
			$this->dataBindModel->addDataBind( $bind );
	...
	function statistics($task, $option)
	{
		...
		JHTML::_('stylesheet', 'backend.css', 'administrator/components/'. $option . '/css/');
		$today = new JDate();
		$this_month = (int)$today->toFormat('%m', true);
		$this_year = (int)$today->toFormat('%Y', true); 
		$db = JFactory::getDBO();
		$statistics = array();
		for($i = 0; $i < 12; $i++)
		{
