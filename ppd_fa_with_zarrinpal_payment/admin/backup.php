...
require_once(JPATH_COMPONENT.DS.'html'.DS.'backup.html.php');
...
class BackupForm extends PPDForm
{
	...
	*/
	...
	function getTmpFile()
	{
		...
		return $config->getValue('config.tmp_path').DS.'back' . $this->getFileNameWithDate() . "bk";
	}
	...
	function decompressFiles($gzFile, &$decompressedFiles)
	{
		...
		$config = JFactory::getConfig();
		$extractDir = $config->getValue('config.tmp_path').DS.'extract';
	...
	function restore()
	{
		...
		else
		{
			jimport('joomla.filesystem.file');
			$config = JFactory::getConfig();
			$tmp_dest 	= $config->getValue('config.tmp_path').DS.$importfile['name'];
