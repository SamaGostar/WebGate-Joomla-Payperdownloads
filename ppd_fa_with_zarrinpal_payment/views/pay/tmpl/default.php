...
if($this->resourceObj)
{
	...
	**/
	...
	<?php
	{
		if($this->usePaypal)
		{
			if($this->paymentInfo->test_mode)
			{
				if($this->paymentInfo->usesimulator)
					$paypal = "index.php?option=com_payperdownloadplus&task=paypalsim&level=payment&type=res";
				else
		...
		<input type="hidden" name="business" value="<?php echo htmlspecialchars($this->paymentInfo->paypal_account);?>"/>
		...
		?>
		<input type="hidden" name="notify_url" value="<?php echo $root;?>index.php?option=com_payperdownloadplus&amp;task=confirmres<?php echo $params;?>"/>
		<?php 
			if($this->thankyouUrl){
			?>
			<input type="hidden" name="return" value="<?php echo $this->thankyouUrl;?>"/>
			<?php }?>
			<input type="hidden" name="no_note" value="1"/>
			<input type="hidden" name="no_shipping" value="1"/>
			<input type="hidden" name="rm" value="2"/>
			<input type="image" src="<?php echo JURI::BASE(); ?>/components/com_payperdownloadplus/css/zarrinpal.jpeg" name="submit" alt="پرداخت زرینپال"/>
...
if($this->multipleLicenseView == 1)
{
	?>
	<input type="hidden" id="licenses_count" value="<?php echo count($this->licenses);?>"/>
	<ul>
	<?php
	$i = 0;
	foreach($this->licenses as $license)
	{
		?>
		<li>
		<input type="hidden" id="license_<?php echo $i++;?>" value="<?php echo $license->license_id;?>"/>
		<a href="" onclick="showLicenseById(<?php echo $license->license_id;?>); return false;"><?php echo htmlspecialchars($license->license_name);?></a>
		</li>
		<?php
	}
	?>
...
foreach($this->licenses as $license)
{
	if(!$license->canRenew)
		continue;
	?>
	<div class="ppd_license" <?php if(count($this->licenses) > 0 && $this->multipleLicenseView == 1 && !$first) 
	{ ?> style="visibility:hidden; position: absolute;"<?php  } ?> id="div_license_<?php echo $license->license_id;?>">
	<br/>
   ...
   ?></span>
	...
	?>
		...
		<input type="hidden" name="option" value="com_payperdownloadplus"/>
	...
	else if($this->user->id)
	{
		if($this->usePaypal)
		{
			...
			{
				if($this->paymentInfo->usesimulator)
					$paypal = "index.php?option=com_payperdownloadplus&task=paypalsim&level=payment&type=lic";
				else
					$paypal = "https://www.sandbox.paypal.com/cgi-bin/webscr";
			}
			else
		...
		$params = "";
		?>
		<input type="hidden" name="notify_url" value="<?php echo $root;?>index.php?option=com_payperdownloadplus&amp;task=confirm<?php echo $params;?>"/>
		<?php 
		$returnParams = "";
		if($this->thankyouUrl){
			...
			if(strstr($this->thankyouUrl, "?") === false)
				...
				$returnParams = "&lid=" . (int)$license->license_id;
			?>
			<input type="hidden" name="return" value="<?php echo $this->thankyouUrl . $returnParams;?>"/>
			<?php }?>
			<input type="hidden" name="no_note" value="1"/>
			<input type="hidden" name="no_shipping" value="1"/>
			<input type="hidden" name="rm" value="2"/>
		...
		if($this->alpha_integration == 2 && $license->aup > 0 && $license->aup <= $this->points)//Use Alpha Points to buy
		{
			...
			<br/>
			<?php
			$link = "index.php?option=com_payperdownloadplus&task=buywithaup&lid=" . 
	...
	<?php
		if($this->show_login)
		{
			require_once(JPATH_COMPONENT . DS . 'views' . DS . 'pay' . DS . 'tmpl' . DS . 'login.php');
		}
		if($this->useQuickRegister)
		{
			echo "<hr/>";
			require_once(JPATH_COMPONENT . DS . 'views' . DS . 'pay' . DS . 'tmpl' . DS . 'register.php');
		}
	}
}
?>
