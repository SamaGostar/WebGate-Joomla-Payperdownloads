...
class ImageVisualDataBind extends VisualDataBind
{
	...
	function renderEdit(&$row)
	{
		...
		if($this->acceptVideos)
		{
			...
			JHTML::script( 'flowplayer.min.js', $scriptPath );
			JHTML::stylesheet( 'style.css', $scriptPath );
		}
		$html = "<tr>" . $this->renderFieldLabel() . "<td>";
		$dataField = $this->dataField;
		if($this->allowBlank)
	...
	function getFileName($name)
	{
		...
		$tmp_dest = $this->imagePath . DS . $file_name . "_" . time() . "_";
		for($i = 0; $i < 16; $i++)
	...
	function onBeforeStore(&$row)
	{
		...
		if($this->allowBlank && $is_null == "on")
		{
			if($row->{$this->dataField})
			{
				...
				$previous_image_path = str_replace("/", DS, $row->{$this->dataField});
				JFile::delete(JPATH_ROOT . DS . $previous_image_path);
			}
		...
		// Move uploaded file
		if(JFile::upload($tmp_src, JPATH_ROOT . DS . $tmp_dest))
		{
			if( !$this->acceptVideos && $this->savePreview )
				$this->saveImagePreview( JPATH_ROOT . DS . $tmp_dest );
			if($row->{$this->dataField})
			{
				//Eliminar archivo anterior
				$previous_image_path = str_replace("/", DS, $row->{$this->dataField});
	...
	function onBeforeDelete($row, $id)
	{
		...
		$db->setQuery("SELECT " . $row->getKeyName() . ", " . $this->dataField . " as image_path FROM " . $row->getTableName() . "
			WHERE " . $row->getKeyName() . " = '" . $db->getEscaped($id) . "'");
		$object = $db->loadObject();
		if(isset($object) && $object != null)
			$this->image_path = str_replace("/", DS, $object->image_path);
		else 
	...
	function onAfterDelete($row, $id)
	{
		...
		jimport('joomla.filesystem.file');
		if($this->image_path)
			JFile::delete(JPATH_ROOT . DS . $this->image_path);
		return true;
	}
