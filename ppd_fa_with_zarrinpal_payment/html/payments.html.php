...
class PaymentsHtmlForm extends BaseHtmlForm
{
	function renderStatistics($statistics, $this_month, $this_year)
	{
		$root = JURI::root();
		?>
		<link rel="stylesheet" href="<?php echo $root;?>administrator/components/com_payperdownloadplus/css/stat.css" type="text/css" />
		<?php
		$max = 0;	
		...
		for($i = -11; $i <= 0; $i++)
		{
			...
			if($height == 0)
				$height = 2;
			$this->renderOneBar($height, $root. 'administrator/components/com_payperdownloadplus/images/green.png');
		}
		?>
		</tr>
		<tr class="stat_paid">
