...
class  plgSystemPayperDownloadPlus extends JPlugin
{
	 ...
	 * @since	1.0
	 */
	function plgSystemPayperDownloadPlus(& $subject, $config)
	{
	...
	*/
	function onAfterRoute()
	{
		...
		$db = JFactory::getDBO();
		$escaped_option = $db->getEscaped($option);
		...
		if(count($resources) > 0)
		{
			...
			if($requiresPayment)
			{
				...
				{
					$link = "index.php?option=com_payperdownloadplus&view=noaccess";
				}
				else
				{
					$link = "index.php?option=com_payperdownloadplus&view=pay&m=1&h=1&lid=" . $licenses;
					if(count($resourcesId) > 0)
					{
	...
	function checkResourceForUser($resource_id, $item_id)
	{
		...
		if($user_id)
		{
			...
			if($item_id != null && $item_id != 0)
				$query .= " AND (item_id = '" . $db->getEscaped($item_id) . "' OR item_id IS NULL)";
			else
	...
	*/
	...
	function showLicensesPrices()
	{
		JLoader::register('PayPerDownloadPrices', 
			JPATH_ADMINISTRATOR . DS . "components" . DS . "com_payperdownloadplus" . DS . "classes" . DS . "prices.php");
	...
	/*
	Shows users owned valid licenses on tag and  on Kunena forum
	*/
